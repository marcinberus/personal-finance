...
module.exports = async function globalSetup() {
  ...
  connectionUrl.searchParams.set('schema', 'reporting');
  connectionUrl.searchParams.set('options', '-c search_path=reporting');
  ...
  execSync('npx prisma migrate deploy --config apps/reporting-service/prisma.config.ts', {
    stdio: 'inherit',
    cwd: path.join(__dirname, '..', '..', '..', '..', '..'),
    env: {
      ...process.env,
      DATABASE_URL: connectionString,
      REPORTING_DATABASE_URL: connectionString,
    },
  });
